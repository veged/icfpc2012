...
Solver.prototype.run = function run(callback) {
    ...
    function step(depth) {
        ...
        ['U','D','L','R','W'].forEach(function(cmd) {
            ...
            console.log(child.map.stringify());
            console.log(child.map.moves, child.map.score);
        }, this);

        // XXX: Limit queue
        if (this.queue.length > 10000 && this.depth > this.cleanupDepth) {
            var q = [];

            // Sort queue (TODO: Use optimized structure here)
            this.queue.sort(function(a, b) {
                return a.depth > b.depth ? 1 : a.depth < b.depth ? -1 :
                       a.phi > b.phi ? -1 : a.phi < b.phi ? 1 :
                       a.cmd > b.cmd ? -1 : a.cmd < b.cmd ? 1 : 0;
            });

            this.queue = this.queue.slice(0, 2000);
            this.cleanupDepth = this.depth;
        }

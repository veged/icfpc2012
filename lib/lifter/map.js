...
function Map(source, config) {
    if (source instanceof Map) {
        ...
        // Set player's life
        this.life = this.config.waterproof;
...
Map.prototype.move = function move(pairs, iteration) {
    ...
    sets.length && (clone.revision = iteration);

    return clone.update(sets);
};
...
Map.prototype.step = function step() {
    ...
    var player = clone.player,
        hasAbove = moves.some(function(move) {
            return move.to.x === player.x && (move.to.y + 1) === player.y;
        });
    ...
    if (clone.player.y >= clone.waterLevel) {
        ...
        clone.life = clone.config.waterproof;
    }
...
Map.prototype.playerAction = function playerAction(cmd) {
    ...
    if (clone.lambdas.length === 0) {
        clone = clone.update([{ pos: clone.lift, value: 'O' }])
    }
...
function createTiles (source) {
    source = source.split(/\n/g);
    function doCreateTiles (pos1, pos2) {
        if (pos1.x === pos2.x || pos1.y === pos2.y ) {
            return new Tile('', pos1, pos2)
        }

        if (pos2.x - pos1.x === 1 && pos2.y - pos1.y === 1) {
            return new Tile (source[pos1.y][pos1.x], pos1, pos2);
        }
        ...
        return new Tile(
            [
                doCreateTiles(pos1, pos3),
                doCreateTiles({ x: pos3.x, y: pos1.y }, { x: pos2.x, y: pos3.y }),
                doCreateTiles({ x: pos1.x, y: pos3.y }, { x: pos3.x, y: pos2.y }),
                doCreateTiles(pos3, pos2),
            ],
            pos1,
            pos2)
    }

    return doCreateTiles({ x: 0, y: 0 }, { x: source[0].length, y: source.length });
}

function Tile (source, pos1, pos2) {
    this.pos1 = pos1;
    this.pos2 = pos2;
...
Tile.prototype.update = function update(sets) {
    ...
    if (this.data) {
        ...
        return new Tile(set.value, this.pos1, this.pos2);
    }

    var subsets = [[], [], [], []],
        pos1 = this.pos1,
        pos2 = this.pos2,
        pos3 = this.subtiles[0].pos2;
...
Tile.prototype.get = function get(pos) {
    if (pos.x < this.pos1.x || pos.x >= this.pos2.x ||
        pos.y < this.pos1.y || pos.y >= this.pos2.y) throw '' + pos.x + ':' + pos.y + ' is outside tile';
    if (this.data) return this.data;

    var X = 0;
    if (pos.x >= this.pos1.x + (this.width >> 1)) {
        X = 1;
    }
    var Y = 0;
    if (pos.y >= this.pos1.y + (this.height >> 1)) {
        Y = 1;
    }

...
function Map(source, config) {
    if (source instanceof Map) {
        ...
        // Player moves
        // XXX: Use real tile depth
        this.moves = new Array(
            Math.ceil(Math.log(Math.max(this.width, this.height)) /
                      Math.log(2))).join('.').split('.');
    }
};
...
exports.fromString = function fromString(str) {
    ...
    return new Map(conf[0].replace(/\n$/, '').split(/\n/g),
        ...
        if (/^Water /.test(line)) {
            ...
            return ['trampoline' + match[1], match[2]];
        }
        return null;
    })
...
Map.prototype.playerAction = function playerAction(cmd) {
    ...
    if (!(/[#RL1-9]/).test(newPosBlock)) {
        if (newPosBlock === '*' || newPosBlock === '@') {
            ...
            sets.push({ pos: pos, value: ' ' });
            sets.push({
                pos: this.trampolines[this.config['trampoline' + newPosBlock]],
                value: 'R' });
        } else {
            // Move player
            sets.push({ pos: pos, value: ' ' });
...
function Tile (source, pos1, pos2, level) {
    ...
    if (Array.isArray(source)) {
        ...
        source.forEach(function(s) {
            Object.keys(s.trampolines).forEach(function(k) { this.trampolines[k] = s.trampolines[k] }, this);

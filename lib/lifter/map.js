...
Map.prototype.playerAction = function playerAction(cmd) {
    ...
    } else if (newPosBlock !== 'R') return false;
    ...
    for (; w >= 1 && h >= 1; i--, w /= 2, h /= 2) {
        ...
        //if (cmd) {
            //var last = clone.moves[i].length - 1;
            //if (last >= 0 && clone.moves[i][last] === opposite[cmd]) {
                //clone.moves[i] = clone.moves[i].slice(0, -1);
            //} else {
                //clone.moves[i] += cmd;
            //}
        //}
        ...
        var tileNum = Math.floor(curr.x / w) + ':' + Math.floor(curr.y / h);

        if (tileNum)
            clone.moves[i] = (clone.moves[i] || '.')
                .replace(new RegExp('\.' + tileNum + '\.|\.$'), '.' + tileNum + '.');
...
Tile.prototype.update = function update(sets) {
    ...
    var setsHash = JSON.stringify(sets);
    if (this.cache.hasOwnProperty(setsHash)) {
        return this.cache[setsHash];
    }
    ...
    return this.cache[setsHash] = new Tile(
...
Tile.prototype.get = function get(pos) {
    ...
    var _this = this;
    while (true) {
        if (_this.data) return _this.data;
        _this = _this.subtiles[_this.subtileIndex(pos)];
    }

...
function bla (map, level, cmd) {
    var queue = [{ map: map, level: level, cmd: cmd, parent: null }],
        task;

    while(task = queue.shift()) {
        step(task).forEach(function(t) { queue.push(t) });
    }
    ...
    function step (task) {
        ...
        if (tile.data) {
            ...
            return [{ map: map2, level: parent.level, cmd: parent.cmd, parent: parent.parent }];
        } else {
            assert(tile.subtiles);
            var subtileIndex = tile.subtileIndex(map.player),
                res = [];
            blaOptions[''][subtileIndex].forEach(function(cmd2) {
                res.push({ map: map, level: level + 1, cmd: cmd2, parent: task });
            });
            blaOptions[cmd][subtileIndex].forEach(function(cmd2) {
                res.push({ map: map, level: level + 1, cmd: cmd2, parent: parent });
            });
            return res;
        }
    }
